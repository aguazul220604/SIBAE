...
@section('content')
    <br>
    <form id="yearForm" method="GET" action="{{ route('control.eea', ['id_year' => $id_year]) }}">
    ...
    <div class="card shadow-lg">
        <div class="card-body">
            ...
            @endphp
            <form id="mainForm" action="{{ route('control.eeaguardar') }}" method="post">
                @csrf
                ...
                <x-adminlte-datatable id="table1" :heads="$heads" head-theme="dark" :config="$config">
                    @foreach ($puntos as $punto)
                        <tr>
                            <td><input type="hidden" name="idp[]" value="{{ $punto->id }}">{{ $punto->cipe }}</td>

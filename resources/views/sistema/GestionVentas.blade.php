...
@section('content')
    ...
    <div class="card shadow-lg">
        <div class="card-body">
            ...
            <form id="mainForm" action="{{ route('control.ventasguardar') }}" method="post">
                @csrf
                ...
                <x-adminlte-datatable id="table1" :heads="$heads" head-theme="dark" :config="$config">
                    @foreach ($tarifas as $tarifa)
                        <tr>
                            <td><input type="hidden" name="idt[]" value="{{ $tarifa->id }}">{{ $tarifa->codigo }}</td>
                            <input type="hidden" name="idfecha[]" value="{{ $idFecha->id }}">
                            <td>{{ $tarifa->descripcion }}</td>
                            <td>{{ $idFecha->fecha }}</td>
                            <td><input type="number" name="monto[]" step="any"></td>
                            <td></td>
                        </tr>
                    @endforeach

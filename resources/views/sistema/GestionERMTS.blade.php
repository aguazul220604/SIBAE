...
@section('content')
    ...
    <div class="card shadow-lg">
        <div class="card-body">
            ...
            @endphp
            <form id="mainForm" action="{{ route('control.ermsguardar') }}" method="post">
                @csrf
                ...
                <x-adminlte-datatable id="table1" :heads="$heads" head-theme="dark" :config="$config">
                    @foreach ($energiaData2 as $ERMTS2)
                        <tr>
                            <input type="hidden" name="ids[]" value="{{ $ERMTS2->ids }}">
                            <input type="hidden" name="idfecha[]" value="{{ $idFecha->id }}">
                            <td>{{ $ERMTS2->cipe }}</td>
                            <td>{{ $ERMTS2->subestacion }}</td>
                            <td>{{ $ERMTS2->punto }}</td>
                            <td>{{ $ERMTS2->medidor }}</td>
                            <td>{{ $ERMTS2->rtc }}</td>
                            <td>{{ $ERMTS2->rtp }}</td>
                            <td>{{ $idFecha->fecha }}</td>

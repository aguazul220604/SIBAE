...
@section('content')
    ...
    <br>
    <form id="mesForm" method="GET" action="{{ route('control.ventas', ['id_year' => $id_year]) }}">
    ...
    <br>
    @php
        echo "<h1>Administración de ventas anuales: 20$id_year</h1>";
        echo '<h3>Tarifas de venta mensuales</h3>';
    @endphp
    <br>
    <div class="card shadow-lg">
        <div class="card-body">
            @php
                $heads = ['Código', 'Tarifa', 'Fecha', 'Monto'];
                $config = [
                    'language' => [
            ...
            @endphp
            <form id="mainForm" action="{{ route('control.ventasactualizar') }}" method="post">
                @csrf
                ...
                <x-adminlte-datatable id="table1" :heads="$heads" head-theme="dark" :config="$config">
                    @foreach ($tarifaVentas as $tarifaVenta)
                        <tr>
                            <input type="hidden" name="id[]" value="{{ $tarifaVenta->id }}">
                            <input type="hidden" name="idfecha[]" value="{{ $idFecha->id }}">
                            <td>{{ $tarifaVenta->id_tarifa }}</td>
                            <td>{{ $tarifaVenta->descripcion }}</td>
                            <td>{{ $idFecha->fecha }}</td>
                            <td><input type="number" name="monto[]" step="any" value="{{ $tarifaVenta->monto }}"></td>
                            <td></td>
                        </tr>
                    @endforeach

...
@section('content')
    ...
    <br>
    <form id="yearForm" method="GET" action="{{ route('control.erms', ['id_year' => $id_year]) }}">
    ...
    <br>
    @php
        echo "<h1>Administración de Energías Recibidas (Subestaciones/Bancos): 20$id_year</h1>";
    ...
    <div class="card shadow-lg">
        <div class="card-body">
            @php
                $heads = ['CIPE', 'Subestación', 'Punto', 'Medidor', 'RTC', 'RTP', 'Fecha', 'Energía'];
                $config = [
                    'language' => [
            ...
            @endphp
            <form id="mainForm" action="{{ route('control.ermsactualizar') }}" method="post">
                @csrf
                ...
                <x-adminlte-datatable id="table1" :heads="$heads" head-theme="dark" :config="$config">
                    @foreach ($energiaData as $ERMTS)
                        <tr>
                            <input type="hidden" name="ide[]" value="{{ $ERMTS->ide }}">
                            <input type="hidden" name="idfecha[]" value="{{ $idFecha->id }}">
                            <td>{{ $ERMTS->cipe }}</td>
                            <td><input type="hidden" name="subestacion[]" id="subestacion"
                                    value="{{ $ERMTS->ids }}">{{ $ERMTS->subestacion }}</td>
                            <td>{{ $ERMTS->punto }}</td>
                            <td>{{ $ERMTS->medidor }}</td>
                            <td>{{ $ERMTS->rtc }}</td>
                            <td>{{ $ERMTS->rtp }}</td>
                            <td>{{ $idFecha->fecha }}</td>
                            <td><input type="number" name="monto[]" id="monto" step="any"
                                    value="{{ $ERMTS->energias }}"></td>
                            <td></td>
                        </tr>
                    @endforeach
            ...
            <div class="col-md-7 text-right">
                <button type="button" class="btn btn-primary custom-btn" id="actualizarBtn">Actualizar registros</button>
            </div>
            <br>
        </div>
    </div>
